...
def ipython_display_max_rows(rows: int | None = None):
    import pandas as pd
...
def highlight_source(
    ...
    *,
    ...
    match output_fmt:
        ...
        case _:
            raise ValueError(
                f"output_fmt must be 'html' or 'terminal', but got {output_fmt}.")
...
def show_source(
    ...
    *,
    ...
    if inspect.isbuiltin(obj):
        print("builtin, no source code available")
        return

    code = inspect.getsource(obj)
    highlight_code = highlight_source(
        code, style=style, language=language, output_fmt=output_fmt)
    # Highlight
    match output_fmt:
        case "html":
